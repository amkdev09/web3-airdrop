...
export default function AirdropPage() {
  ...
  const { data: data, isLoading } = useQuery({
    ...
    queryFn: () =>
      Promise.all([
        userServices.getVaultSummary(),
        userServices.getVaultUser(),
      ]),
  });
  const [vaultSummary, vaultUser] = data ?? [];
        ...
        <div>
          <button className="cursor-pointer">
            <svg
              ...
              aria-hidden="true"
              role="img"
              className="iconify iconify--ic size-7 text-[#009C8A]"
              width="1em"
              height="1em"
              viewBox="0 0 24 24"
            >
            ...
            <div className="absolute inset-0 p-4 flex justify-between">
              <div className="flex items-start flex-col">
                <p className="text-base text-[#D9D9D9]">
                  Rank: {isLoading ? "Loading..." : (vaultSummary?.rank ?? "0")}
                </p>
                <div className="space-y-0.5 sm:space-y-2 mt-4 sm:mt-5">
                  <p className="text-sm text-gray-400">Total Earning</p>
                  <p className="text-2xl sm:text-4xl text-[#D9D9D9]">
                    {isLoading
                      ? "Loading..."
                      : vaultSummary?.availableIncome
                        ? `$${vaultSummary?.availableIncome}`
                        : "$0"}
                  </p>
                </div>
              ...
              <div className="mb-5">
                <p className="font-wavacorp text-base tracking-[0.15em]">
                  UltraDefi
                </p>
              </div>
        ...
        <div className="flex flex-col gap-y-4">
          <div className="flex items-center gap-2 text-[#D9D9D9] text-sm cursor-pointer h-14 border border-white/5 bg-white/10 backdrop-blur-md w-full rounded-xl px-3">
            <svg
              ...
              aria-hidden="true"
              role="img"
              className="iconify iconify--solar size-5 shrink-0"
              width="1em"
              height="1em"
              viewBox="0 0 24 24"
            >
            ...
            <p className="text-sm break-all">
              {address || "Wallet address not set."}
            </p>
          </div>
          <div>
            <div className="team-radio-group my-5">
              <input
                type="radio"
                name="teamPlan"
                id="team-glass-personal"
                checked={selectedTeam === "personal"}
                onChange={() => handleSelectTeam("personal")}
              />
            ...
            <div className="grid grid-cols-[1fr_auto_1fr] gap-x-2 items-stretch">
              <button
                ...
                className="flex w-full min-w-0 justify-center items-center bg-gradient-to-b from-[rgba(255,135,149,0.7)] to-[rgba(0,156,138,0.7)] font-wavacorp uppercase tracking-widest text-sm sm:text-base h-14 rounded-xl shadow-2xl disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              >
                Withdraw
              </button>
              <button
                type="button"
                className="flex justify-center items-center shrink-0 px-3 sm:px-4 bg-gradient-to-b from-[rgba(190,135,255,0.7)] to-[rgba(0,156,138,0.7)] font-wavacorp uppercase tracking-widest text-sm sm:text-base h-14 rounded-xl shadow-2xl disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              >
                ${vaultUser?.incomeBalance ?? "0"}
              </button>
              <button
          ...
          <div>
            <p className="text-base font-medium text-white mb-3">
              {selectedTeam === "team" ? "Team" : "Personal"} stats
            </p>
            <p className="text-xs text-white/50 mb-3">
              Your referral {selectedTeam === "team" ? "team" : "personal"}{" "}
              ...
              {selectedTeam === "team" ? (
                <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-3 items-baseline">
                  ...
                  <span className="text-base sm:text-lg font-semibold text-white tabular-nums text-right">
                    {isLoading
                      ? "…"
                      : vaultSummary?.activeTeamBusiness
                        ? `$${vaultSummary.activeTeamBusiness}`
                        : "$0"}
                  </span>
                  <span className="text-sm text-white/50">Team size</span>
                  <span className="text-base sm:text-lg font-semibold text-white tabular-nums text-right">
                    {isLoading ? "…" : (vaultSummary?.teamCount ?? "0")}
                  </span>
                </div>
              ) : (
                <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-3 items-baseline">
                  <span className="text-sm text-white/50">Rank Income</span>
                  <span className="text-base sm:text-lg font-semibold text-white tabular-nums text-right">
                    {isLoading
                      ? "…"
                      : vaultUser?.totalRankIncome
                        ? `$${vaultUser.totalRankIncome}`
                        : "$0"}
                  </span>
                  <span className="text-sm text-white/50">Roi Income</span>
                  <span className="text-base sm:text-lg font-semibold text-white tabular-nums text-right">
                    {isLoading
                      ? "…"
                      : vaultUser?.totalRoiIncome
                        ? `$${vaultUser.totalRoiIncome}`
                        : "$0"}
                  </span>
                  <span className="text-sm text-white/50">Level Income</span>
                  <span className="text-base sm:text-lg font-semibold text-white tabular-nums text-right">
                    {isLoading
                      ? "…"
                      : vaultUser?.totalLevelIncome
                        ? `$${vaultUser.totalLevelIncome}`
                        : "$0"}
                  </span>
                  ...
                  <span className="text-base sm:text-lg font-semibold text-white tabular-nums text-right">
                    {isLoading
                      ? "…"
                      : vaultUser?.totalGlobalPoolIncome
                        ? `$${vaultUser.totalGlobalPoolIncome}`
                        : "$0"}
                  </span>
                </div>
        ...
        <div className="space-y-6">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-base text-white">Withdraw</p>
              <p className="text-base text-white">Deposit</p>
            </div>
            <div className="space-y-4">
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center justify-center size-10 shrink-0 rounded-full bg-white/10">
                    <svg
                      ...
                      aria-hidden="true"
                      role="img"
                      className="iconify iconify--fluent size-5 text-white"
                      width="1em"
                      height="1em"
                      viewBox="0 0 24 24"
                    >
                  ...
                  <div className="space-y-1">
                    <h3 className="text-base text-white">
                      Watch Selsila Premium Youtube
                    </h3>
                    <p className="text-xs text-[#D9D9D9]">+0.3 SELSI</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <button className="text-sm px-4 h-8 rounded-full bg-white/10 hover:bg-white/20 text-white transition-colors disabled:opacity-50">
                    Watch
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between w-full p-4 rounded-xl border border-selsila-purple bg-white/5 backdrop-blur-md gap-5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-1">
                    <h3 className="text-base text-white">
                      Watch Video on Selsila Instagram
                    </h3>
                    <p className="text-xs text-[#D9D9D9]">+0.2 SELSI</p>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between w-full p-4 rounded-xl border border-selsila-purple bg-white/5 backdrop-blur-md gap-5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-1">
                    <h3 className="text-base text-white">Welcome Rank</h3>
                ...
                <div className="text-sm flex items-center justify-center size-8 rounded-full bg-green-500/20 text-green-500">
                  <svg
                    ...
                    aria-hidden="true"
                    role="img"
                    className="iconify iconify--material-symbols size-6"
                    width="1em"
                    height="1em"
                    viewBox="0 0 24 24"
                  >
              ...
              <div className="flex items-center justify-between w-full p-4 rounded-xl border border-selsila-purple bg-white/5 backdrop-blur-md gap-5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-1">
                    <h3 className="text-base text-white">
                      Subscribe Selsila Youtube Channel
                    </h3>
                    <p className="text-xs text-[#D9D9D9]">+0.6 SELSI</p>
                  </div>
                </div>
